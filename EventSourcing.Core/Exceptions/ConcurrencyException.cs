...
namespace EventSourcing.Core.Exceptions
{
    public class ConcurrencyException : EventStoreException
    {
        ...
        public ConcurrencyException(Event e) : base(
            $"Event '{e.Type}' with Version {e.AggregateVersion} already exists for Aggregate '{e.AggregateType}' with Id '{e.AggregateId}'")
        {
        }
    }
}